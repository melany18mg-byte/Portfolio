...
def normalize_key(row):
    parts = [
        ...
        row.get("single_combined_molecule","")
    ]
    parts = [normalize_product(p) for p in parts]
    return "_".join(parts)

df_final["key"] = df_final.apply(normalize_key, axis=1)
df_dash["key"]  = df_dash.apply(normalize_key, axis=1)
